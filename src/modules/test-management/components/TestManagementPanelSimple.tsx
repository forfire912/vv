...
export const TestManagementPanelSimple: React.FC<TestManagementPanelSimpleProps> = ({
  ...
  const [executionUpdates, setExecutionUpdates] = useState(0);
  ...
  const handleExecutionStart = () => {
    ...
    // 触发执行面板更新
    setExecutionUpdates(prev => prev + 1);
  };
  ...
  return (
    <div className="test-management-panel" style={{
      ...
      <div style={{
        ...
        {activeTab === 'tests' && <TestCasePanelEnhanced onExecutionStart={handleExecutionStart} />}
        {activeTab === 'suites' && <TestSuitePanelSimple />}
        {activeTab === 'results' && <TestExecutionPanelEnhanced key={executionUpdates} />}
      </div>
    </div>
  );
};
